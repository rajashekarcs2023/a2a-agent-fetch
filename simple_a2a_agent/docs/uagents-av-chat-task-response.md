...
**Location:** `uagents_core/adapters/a2a/agentverse_sdk.py` → **`AgentverseA2AStarletteApplication._chat`**, in the block that parses **`resp.body`** after `super()._handle_requests(a2a_request)` (in an installed venv this is usually under `site-packages/uagents_core/.../agentverse_sdk.py`). Prefer changing **`uagents-core`** in its own repo and releasing, not editing `site-packages` long term.
